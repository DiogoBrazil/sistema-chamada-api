...
@injectable()
export class AddProfessionalToHealthUnitUseCase {
  ...
  async execute(healthUnitId: number, professionalId: number): Promise<HealthUnit> {
    ...
    // Verificar se o profissional já está associado à unidade
    const isProfessionalInUnit = healthUnit.professionals.some(p => p.id === professionalId);
    if (isProfessionalInUnit) {
      throw new Error("Professional is already linked to this health unit");
    }
    
    return this.healthUnitRepository.addProfessionalToHealthUnit(healthUnitId, professionalId);
  }
}