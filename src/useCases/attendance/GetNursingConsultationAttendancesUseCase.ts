...
@injectable()
export class GetNursingConsultationAttendancesUseCase {
  ...
  async execute(healthUnitId: number, professionalId: number): Promise<Attendance[]> {
    ...
    return this.attendanceRepository.getAttendances({stage: AttendanceStage.NURSING_CONSULTATION, healthUnitId});
  }
}