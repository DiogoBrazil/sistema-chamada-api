...
@injectable()
export class GetDentalConsultationAttendancesUseCase {
  ...
  async execute(healthUnitId: number, professionalId: number): Promise<Attendance[]> {
    ...
    return this.attendanceRepository.getAttendances({stage: AttendanceStage.DENTAL_CONSULTATION, healthUnitId});
  }
}